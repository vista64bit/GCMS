	...
	if (MAIN_INIT == 'admin' && $isMember) {
		...
		if (!$index) {
			...
			foreach ($config['languages'] AS $item) {
				$content[] = '<li><a id=tab_detail_'.$item.' href="{URLQUERY?module=document-write&qid='.$index['id'].'&tab=detail_'.$item.'}">{LNG_DETAIL}&nbsp;<img src='.DATA_URL.'language/'.$item.'.gif alt='.$item.'></a></li>';
			}
			$content[] = '<li><a id=tab_options href="{URLQUERY?module=document-write&qid='.$index['id'].'&tab=options}">{LNG_OTHER_DETAILS}</a></li>';
			$content[] = '</ul>';
			$content[] = '</div></div>';
			$content[] = '</header>';
			// ฟอร์มเขียน-แก้ไข
			...
			foreach ($config['languages'] AS $language) {
				...
				$content[] = '<span class="g-input icon-document"><input type=text id=write_relate_'.$language.' name=write_relate_'.$language.' value="'.$item['relate'].'" maxlength=64 title="{LNG_RELATE_COMMENT}"></span>';
