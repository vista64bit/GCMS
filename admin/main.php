<?php
	...
	if (MAIN_INIT == 'admin' && $canAdmin) {
		...
		$admin_menus['sections']['help'] = array('0', '<a href="'.WEB_URL.'/admin/index.php?module=accesskey" accesskey=0 title="{LNG_ACCESSKEY}"><span>{LNG_HELP}</span></a>');
		// settings
		...
		foreach ($install_owners AS $text => $items) {
			...
			if (is_file(ROOT_PATH."modules/$text/admin_inint.php")) {
				...
				if (isset($admin_menus['modules'][$text]) && sizeof($admin_menus['modules'][$text]) == 0) {
		...
		foreach ($admin_menus['sections'] AS $section => $name) {
			...
			if (isset($admin_menus[$section]) && sizeof($admin_menus[$section]) > 0) {
				$menus[] = '<ul>';
